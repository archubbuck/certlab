      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <Badge variant="outline" className="ml-2">500 Questions</Badge>
          </div>
        </CardHeader>
  ...
  return (
    <div className="container max-w-6xl mx-auto p-6 space-y-6">
      ...
      <Alert>
        ...
        <AlertDescription>
          Each sample dataset contains 500 practice questions across multiple domains. 
          Importing will add questions to your local database. If questions already exist, 
          use the "Clear" button first to remove old data before re-importing.
        </AlertDescription>
      </Alert>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-medium">
                  {Math.round((fileImportState.progress.current / fileImportState.progress.total) * 100)}%
                </span>
              </div>
              <Progress 
                value={(fileImportState.progress.current / fileImportState.progress.total) * 100} 
                className="h-2" 
              />
            </div>
          )}
                ...
                {fileImportState.result.success ? (
                  <div className="space-y-1 mt-2">
                    ...
                    {fileImportState.result.subcategoriesCreated > 0 && (
                      <div>✓ {fileImportState.result.subcategoriesCreated} subcategories created</div>
                    )}
                  </div>
                ) : (
                  <div className="mt-2">
