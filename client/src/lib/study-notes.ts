...
function extractKeyConcepts(questionText: string): string {
  ...
  if (concepts.includes('incident') || concepts.includes('response'))
    return 'Incident Response & Management';
  if (concepts.includes('continuity') || concepts.includes('business'))
    return 'Business Continuity Planning';
  if (concepts.includes('monitoring')) return 'Security Monitoring & Analytics';
  if (
    concepts.includes('confidentiality') ||
    concepts.includes('integrity') ||
    concepts.includes('availability')
  )
    return 'CIA Triad Principles';
  ...
  if (concepts.includes('vulnerability') || concepts.includes('threat'))
    return 'Vulnerability & Threat Assessment';
  if (concepts.includes('audit') || concepts.includes('log')) return 'Audit & Logging';
  if (concepts.includes('policy') || concepts.includes('procedure'))
    return 'Security Policies & Procedures';
