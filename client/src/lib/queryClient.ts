...
export const queryKeys = {
  ...
  user: {
    ...
    achievementProgress: (userId: string | undefined) =>
      ['/api', 'user', userId, 'achievement-progress'] as const,
    practiceTestAttempts: (userId: string | undefined) =>
  ...
  quiz: {
    ...
    questions: (quizId: number | string | undefined) =>
      ['/api', 'quiz', quizId, 'questions'] as const,
  },
  ...
  // Study Notes queries
  studyNotes: {
    all: (userId: string | undefined) => ['/api', 'study-notes', userId] as const,
    detail: (noteId: number | string | undefined) =>
  ...
  admin: {
    tenants: {
      ...
      stats: (tenantId: number | null | undefined) =>
        ['/api', 'admin', 'tenants', tenantId, 'stats'] as const,
      categories: (tenantId: number | null | undefined) =>
        ['/api', 'admin', 'tenants', tenantId, 'categories'] as const,
      questions: (tenantId: number | null | undefined) =>
        ['/api', 'admin', 'tenants', tenantId, 'questions'] as const,
    ...
    try {
      ...
      if (path.startsWith('/api/user/')) {
        ...
        if (path.includes('/achievement-progress')) {
          ...
          const { allBadges, userBadges, gameStats, userQuizzes } = await getAchievementData(
            userId,
            tenantId
          );
