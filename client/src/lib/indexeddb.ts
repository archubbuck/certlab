...
class IndexedDBService {
    ...
    this.initPromise = new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        if (!db.objectStoreNames.contains(STORES.subcategories)) {
          const subcategoryStore = db.createObjectStore(STORES.subcategories, { keyPath: 'id', autoIncrement: true });
        ...
        if (!db.objectStoreNames.contains(STORES.challenges)) {
          const challengeStore = db.createObjectStore(STORES.challenges, { keyPath: 'id', autoIncrement: true });
        ...
        if (!db.objectStoreNames.contains(STORES.studyGroupMembers)) {
          const memberStore = db.createObjectStore(STORES.studyGroupMembers, { keyPath: 'id', autoIncrement: true });
  ...
  async getOneByIndex<T>(storeName: string, indexName: string, key: IDBValidKey): Promise<T | undefined> {
    const store = await this.getStore(storeName);
    const index = store.index(indexName);
    return new Promise((resolve, reject) => {
