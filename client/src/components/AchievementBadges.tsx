...
export function AchievementBadges({ userId }: AchievementBadgesProps) {
  ...
  const badgesByCategory = badges.reduce((acc, badgeData) => {
  ...
  const currentLevelProgress = (gameStats.totalPoints % gameStats.nextLevelPoints) / gameStats.nextLevelPoints * 100;
          ...
          <CardContent className="pt-4">
            <div className="text-center">
              ...
              <p className="text-purple-700 dark:text-purple-300 font-medium">
                🎉 Congratulations! You've earned {achievements.newBadges} new badge{achievements.newBadges > 1 ? 's' : ''}!
              </p>
            </div>
          </CardContent>
        </Card>
